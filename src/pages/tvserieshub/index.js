...
const TABS = [
  {
    id: "airing_today",
    label: "Airing Today",
    description: "Fresh episodes on air right now",
    icon: (
  ...
  },
  {
    id: "top_rated",
    label: "Top Rated",
    description: "The highest-rated shows of all time",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
  ...
  },
  {
    id: "popular",
    label: "Popular",
    description: "Everyone's talking about these shows",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
  ...
  },
  {
    id: "on_the_air",
    label: "On The Air",
    description: "Currently running series",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
...
export default function TVSeriesHub() {
  ...
  const fetchTVShows = async (category, page = 1, append = false) => {
    try {
      ...
      if (data && data.length > 0) {
        if (append) setTvShows((prev) => [...prev, ...data]);
  ...
  return (
    <>
      <Head>
        <title>TV Series Hub — {activeTabConfig.label} Shows | Veflix</title>
        <meta name="description" content={`${activeTabConfig.description} — Discover TV series on Veflix.`} />
        <meta name="keywords" content="TV shows, TV series, airing today, top rated TV shows, popular TV series, on air, Veflix" />
        <meta property="og:title" content={`TV Series Hub — ${activeTabConfig.label} | Veflix`} />
        <meta property="og:description" content={activeTabConfig.description} />
        <meta property="og:type" content="website" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={`TV Series — ${activeTabConfig.label} | Veflix`} />
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">TV Series</h1>
                </div>
                <p className="text-gray-400 text-sm">{activeTabConfig.description}</p>
              </div>
              {tvShows.length > 0 && !isLoading && (
                <span className="text-gray-500 text-sm shrink-0">{tvShows.length}+ titles</span>
              )}
            </div>
          </div>
        </div>
